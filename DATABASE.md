...
対象範囲（実装済み）: 3章(アカウント), 5章(投稿), 6章(コメント), 7章(返信), 8章(リアクション/いいね), 10章(フォロー), 13章(検索・簡易版), 15章(ジャンル), 18章(フォロー一覧・簡易版)
将来拡張（未実装）: あいまい検索・タグ/放送局/投稿日フィルター, リアクション種別追加（❤️以外）, ブックマーク, 通知, タグ, NGワード, 通報/管理者機能, 放送局
...
検索（SPEC 13章）はタイトル・説明への `ILIKE '%keyword%'` 部分一致で実装（あいまい検索・全文検索インデックスは将来対応）。
並び替えは 新着(created_at)・更新順(updated_at)・人気(reactionsの件数) に対応。
