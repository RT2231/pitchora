...
Version: 0.1.0（MVP: 認証〜投稿〜コメントの範囲のみ）

このドキュメントは Cloudflare D1 上のテーブル設計を定義する。
...
| created_at | TEXT | NOT NULL DEFAULT (datetime('now')) | 作成日時（ISO8601） |
...
| is_edited | INTEGER | NOT NULL DEFAULT 0 | 編集済みフラグ |
| is_deleted | INTEGER | NOT NULL DEFAULT 0 | 論理削除フラグ（削除後は「このコメントは削除されました。」表示） |
| created_at | TEXT | NOT NULL DEFAULT (datetime('now')) | 作成日時 |
| updated_at | TEXT | NOT NULL DEFAULT (datetime('now')) | 更新日時 |
...
- posts: tags(JSON or 別テーブル), broadcast_day, broadcast_time, cast, thumbnail_url
