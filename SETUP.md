...
返信、リアクション種別追加、ブックマーク、通知、タイムライン（人気/おすすめ）、
検索、タグ、放送局、通報/ブロック/ミュート、NGワード、管理者機能など。
`DATABASE.md` の「将来マイグレーション予定」を参照してください。

## 既存プロジェクトへのマイグレーション

フォロー機能を追加した際、`followsテーブル`が新たに必要になりました。
既にNeonでテーブルを作成済みの場合は、Neonの **SQL Editor** で
`worker/migrations/002_follows.sql` の内容を実行してください
